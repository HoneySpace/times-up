...
import { SystemEventsToArgs, TimerEntryTypes, TimerId, TimerTimeEntry, store, subscribe, unsubscribe } from '../../store/store'
...
export const Timer: FC<{ timerId: TimerId }> = ({ timerId }) => {
    ...
    useEffect(() => {
        ...
        const end = (e: SystemEventsToArgs[
            "timer-stopped"
        ]) => {
    ...
    return (
        <div data-running={running} className='p-3 rounded border group data-[running=true]:border-emerald-700 text-slate-600 border-slate-800 hover:border-slate-400 transition-colors shadow-sm shadow-slate-950'>
            <div title={timerId} className='timer-title text-lg font-semibold text-white text-ellipsis overflow-hidden whitespace-nowrap'>
                {timerId}
            </div>
            {((calculated || currentTime) > 0) ? <div>
