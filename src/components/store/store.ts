...
export type SystemEvents = "timer-created" | "timer-started" | "timer-stopped" | "init"
...
export const emit = <E extends SystemEvents>(event: E, args: SystemEventsToArgs[E]) => eventEmitter.emit(event, args)
export const subscribe = <E extends SystemEvents>(event: E, handler: (e: SystemEventsToArgs[E]) => void) => eventEmitter.addListener(event, handler)
export const unsubscribe = <E extends SystemEvents>(event: E, handler: (e: SystemEventsToArgs[E]) => void) => eventEmitter.removeListener(event, handler)
...
export async function init() {
    db = await openDB<DataBase>("db", 1, {
        upgrade(db) {
            ...
            entriesStore.createIndex("by-timer", "timer" satisfies keyof TimerTimeEntry)
            entriesStore.createIndex("by-value", "value" satisfies keyof TimerTimeEntry)
        },
    });
...
function createStore(initialState: TStoreState) {
    ...
    const getAllTimers = async (db: IDBPDatabase<DataBase>) => {
        const all = await db.transaction("timers").objectStore("timers").index("by-use").getAll()
    ...
    subscribe("init", async (db) => {
        ...
        const cursor = await tx.objectStore("timeEntries").index("by-value").openCursor(null, "prev")
    ...
    return {
        ...
        getLastTimerMark: async (timerId: TimerId): Promise<TimerTimeEntry | undefined> => {
            const cursor = await db.transaction("timeEntries").objectStore("timeEntries").index("by-timer").openCursor(timerId, "prev")
            return cursor?.value
        },
        getTimeEntriesAt: async (date: Date) => {
            ...
            const index = db.transaction("timeEntries").objectStore('timeEntries').index("by-value")
        ...
        startTimer(timerId: TimerId) {
            ...
            if (currentState.activeTimer !== null && currentState.activeTimer !== timerId)
...
export const store = createStore({ activeTimer: null, allTimers: [] })
